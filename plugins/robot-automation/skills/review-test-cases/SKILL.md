...
description: >
  ...
  before writing executable Robot Framework test code. It reviews the spec artifact only; it
  does not implement or rewrite tests.
  ...
  Inserted as a quality gate between generate-test-cases and write-robot-code — catches
  gaps, duplication, weak assertions, missing error paths, and invented scenarios before they
  get encoded into test code. Review-only — does NOT modify the spec file, does NOT write
  test code. The write-robot-code skill should be used for implementation.
allowed-tools: Read Glob Grep Task
---

# Review Test Cases

Review TC-ID test case specifications for completeness, accuracy, and quality before they are implemented as executable Robot Framework tests. This is a quality gate — catch problems in the spec, not in the code.
...
- Merging candidates: cases that could be combined into a single data-driven test (`[Template]` keyword) without losing coverage

#### 2e. Implementability

- Flag steps that cannot be automated with Robot Framework + Browser library (e.g., "verify email arrives", "check database directly" — unless a library exists)
...
- **PASS** — no blockers, 2 or fewer warnings. Proceed to write-robot-code.
- **PASS WITH WARNINGS** — no blockers, 3+ warnings. Can proceed but should address warnings.
- **NEEDS REVISION** — 1+ blockers. Do not proceed to write-robot-code until blockers are resolved.
